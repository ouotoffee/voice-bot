...
//voice state update event to check joining/leaving channels
  ...
  if (oldState.channelID && newState.channelID) {
    ...
    async function jointocreatechannel(user) {
      ...
      await user.guild.channels.create(`${user.member.user.username}的房間＃${user.member.user.discriminator}`, {
        type: 'voice',
        parent: "834810227659374613", //or set it as a category id
      }).then(async vc => {
        ...
        await vc.overwritePermissions([
          {
            id: user.id,
            allow: ['MANAGE_CHANNELS','SPEAK','USE_VAD','CONNECT','VIEW_CHANNEL','MUTE_MEMBERS','DEAFEN_MEMBERS','MOVE_MEMBERS'],
          },
          {
            id: user.guild.id,
